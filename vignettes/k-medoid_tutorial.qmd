...
sfkm = fastkmed(sedist, ncluster = 3, iterate = 50)
...
sd = as.tibble(t(sd)) %>%  select(-V4) %>%  mutate(clusters = 1:3)
...
plot_data = cbind(long[c(1:2,5:6,9:10),c(1,3)],long[-c(1:2,5:6,9:10),c(3)])
colnames(plot_data)[2:3] = c("mean","sd")
plot_data = as_tibble(plot_data) %>%
            mutate(type = rep(c("early","mid"),3))
